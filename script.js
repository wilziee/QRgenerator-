document.addEventListener('DOMContentLoaded', () => {
    ...
    function getFormValues(type) {
        ...
        try {
            switch(type) {
                case 'url':
                    textToEncode = document.getElementById('inp-url').value;
                    break;
                case 'text':
                    textToEncode = document.getElementById('inp-text').value;
                    break;
                case 'whatsapp':
                    ...
                    break;
                case 'phone':
                    textToEncode = `tel:${document.getElementById('inp-phone').value}`;
                    break;
                case 'email':
    ...
    function showToast(message) {
        ...
        // Remove toast after animation finishes (3s total)
        setTimeout(() => {
            toast.remove();
        }, 3000);
    }
    ...
    btnReset.addEventListener('click', () => {
        renderInputs(qrTypeSelect.value); // Re-render to clear inputs
    ...
    btnGenerate.addEventListener('click', () => {
        ...
        if (!textData) {
            showToast('⚠️ Mohon isi data dengan benar');
            return;
        }
        ...
        // Dim the container and show loading
        qrPreviewContainer.style.background = 'rgba(255,255,255,0.1)';
        loadingState.classList.remove('hidden');

        // Simulate processing for ~1s
        setTimeout(() => {
            loadingState.classList.add('hidden');
            qrPreviewContainer.style.background = '#fff'; // Revert to white for scanning

            // Generate using qrcode.js (Library from CDN)
            const qrOptions = {
