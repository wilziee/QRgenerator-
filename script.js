...
// Global vars untuk menyimpan raw data QR
...
btnGenerate.addEventListener('click', async () => {
    ...
    if (!data) {
        showToast("⚠️ Mohon isi field wajib terlebih dahulu.");
        return;
    }

    currentQRDataString = data;

    // UI States: Tampilkan loading, sembunyikan preview
    qrPlaceholder.classList.add('hidden');
    qrPreviewContainer.classList.add('hidden');
    dlActions.classList.add('hidden');
    qrLoading.classList.remove('hidden');

    // Artificial delay agar animasi terasa smooth & premium
    setTimeout(async () => {
        try {
            // Generate Canvas (Untuk Preview & PNG)
            ...
            // Generate SVG string (Untuk Download SVG)
            currentQRSvgString = await QRCode.toString(data, {
                type: 'svg',
                width: 1000, // Resolusi tinggi untuk SVG
                margin: 2
// ================= BACKGROUND ANIMATIONS =================
function createShootingStars() {
    const container = document.querySelector('.shooting-stars-container');
...
btnGenerate.addEventListener('click', () => {
    // Pastikan library QRCode sudah di-load browser
    if (typeof QRCode === 'undefined') {
        showToast("❌ Library QR Code belum termuat. Pastikan ada internet.");
        return;
    }
